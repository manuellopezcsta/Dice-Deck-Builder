...
public class SFXManager : MonoBehaviour
{
    ...
    void OnDisable()
    {
        // Aseg�rate de desuscribirte del evento al desactivar el script para evitar fugas de memoria
        SceneManager.sceneLoaded -= OnSceneLoaded;
    }
    void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        // Dependiendo de la escena cargada, cambia la canci�n principal (main song)
        string sceneName = scene.name;
        
        // Detiene la canci�n actual si est� reproduci�ndose
        ...
        switch (sceneName)
        {
            ...
            default:
                PlaySong(MSName.MENU); // Establecer una canci�n predeterminada para otras escenas no definidas
                break;
        }
    }
    ...
    {
        float fadeDuration = 1.0f; // Duraci�n del fade-out en segundos
        ...
        // Seteamos los valores y reproducimos la nueva canci�n
        mainSong.clip = mainSongLibrary[song].Item1;
        mainSong.volume = 0f;
        mainSong.Play();
